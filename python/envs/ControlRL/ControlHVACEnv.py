...
class AllVar(gym.Env):
    ...
    def __init__(self):
        ...
        self.max_TZ = cfg["max_TZ"]  # 35
        self.min_TZ = cfg["min_TZ"]  # 8
        self.min_Tin = cfg["min_Tin"]  # 5
        self.max_Tin = cfg["max_Tin"]  # 30
        ...
        self.MSE_thresh1 = (
            cfg["thresh1"] * self.scale_var
        ) ** 2  # defines threshold for MSE
    ...
    def reset(self):
        ...
        low = np.array([self.min_TZ, self.min_TZ, 1])  # [8, 8, 1]
        high = np.array([self.max_TZ, self.max_TZ, 2])  # [35, 35, 1]
        if cfg["random_output"] == True:
            self.TOUT1_target = self.np_random.uniform(self.min_TZ, self.max_TZ)
        else:
            self.TOUT1_target = cfg["TOUT1_target"]  # 22
        # generate random state between low and high for shape [3,]
